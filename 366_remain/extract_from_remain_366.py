...
def transform_xl_to_json (path = '',sheet_name = 'Sheet1' , name_report = 'Остатки', name_pharm_chain = '36,6') -> dict:
    loger = LoggingMixin().log
    try:
        df = pd.read_excel(path , sheet_name)
        df = df.astype(str)
        loger.info(f'Успешно получено {df[df.columns[0]].count()} строк!')
        df_drugstore = df[[ 'ID аптеки', 'Адрес аптеки']].drop_duplicates()
        df_product = df[['SKU Наименование', 'SKU ID']].drop_duplicates()
   

        df_drugstore['hash_drugstore'] = df_drugstore.apply(lambda _: '', axis=1)
        ...
        df = df.merge(df_drugstore, on = ['ID аптеки', 'Адрес аптеки'], how = 'left')
        df = df.merge(df_product, on = ['SKU Наименование', 'SKU ID'], how = 'left')
        df_report = df[['uuid_report', 'Период', 'Количество, уп', 'hash_drugstore', 'hash_drugstore_addr', 'hash_product']]
        df_report.rename(columns = {'Период':'period', 'Количество, уп':'quantity'}, inplace=True)
        ...
        df_product.rename(columns={'SKU Наименование':'name', 'SKU ID':'id'}, inplace=True)
