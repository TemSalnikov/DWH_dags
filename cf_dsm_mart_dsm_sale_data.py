...
def check_and_split_date_range(start_date, end_date):
    ...
    try:
        if not isinstance(start_date, pendulum.DateTime) and not isinstance(end_date, pendulum.DateTime):
...
def cf_dsm_mart_dsm_sale_data():
    ...
    def trigger_dags(periods, **kwargs):
        for period in periods:
            ...
            logger.info(f"RUN_ID: triggered_by_{short_id}_{kwargs['dag_run'].run_id}")
            trigger = trigger_dag(
