...
@dag(
    dag_id='cf_xls_kafka_mart_fpc_mz_remain',
    default_args=default_args,
    schedule_interval="@monthly",
    catchup=False,
    params = {'directory': '/opt/airflow/data/reports/МЗ/Остатки/',
              'name_report': 'Остатки',
              'name_pharm_chain': 'Мелодия Здоровья',
              'prefix_topic': 'fpc_mz'
            },
    tags=['advanced', 'mz']
)
def cf_xls_kafka_mart_fpc_mz_remain():
    @task
    ...
    def trigger_or_skip(parametrs: Optional[Dict], processing_files: Optional[Dict], **context):
        ...
        if processing_files:
            ...
            _dag_id = str(context["dag"]).split(':')[1].strip().strip('>')
            loger.info(f'Успешно получено dag_id {_dag_id}!')
            TriggerDagRunOperator(
                task_id=f"trigger_wf_mz_remain",
                trigger_dag_id='wf_xls_kafka_mart_fpc_mz_remain',
                conf={_dag_id[3:]: parametrs}
            ).execute(context)
        else:
            raise AirflowSkipException("Новых файлов для обработки не найдено.")
...
cf_xls_kafka_mart_fpc_mz_remain()