...
CREATE VIEW stg.v_sn_mart_dcm_data
AS 
SELECT 
    cd_reg,
    cd_u,
...
CREATE FUNCTION stg.v_sv_mart_dcm_data(p_processed_dttm DateTime)
RETURNS TABLE
AS 
    SELECT 
        cd_reg,
        ...
        max(processed_dttm) as processed_dttm
    FROM stg.mart_dcm_data
    WHERE processed_dttm <= p_processed_dttm
    GROUP BY cd_reg, cd_u, stat_year, stat_month, sales_type_id, effective_dttm
    HAVING deleted_flag = false;

/* 3. Аксессор получения инкремента для */
CREATE FUNCTION stg.v_iv_mart_dcm_data(p_from_dttm DateTime, p_to_dttm DateTime)
RETURNS TABLE
AS 
    SELECT 
        cd_reg,
        ...
        max(processed_dttm) as processed_dttm
    FROM stg.mart_dcm_data
    WHERE processed_dttm between p_from_dttm and p_to_dttm
    GROUP BY cd_reg, cd_u, stat_year, stat_month, sales_type_id, effective_dttm
    HAVING deleted_flag = false;
    ...
    /* 1. Аксессор получения актуального среза на конкретную дату обработки*/
CREATE VIEW stg.v_sn_mart_dcm_dict
AS 
SELECT 
    cd_u,
    ...
    argMax(hash_diff, processed_dttm) as hash_diff,
    max(processed_dttm) as processed_dttm
FROM stg.mart_dcm_dict
GROUP BY 
    cd_u
HAVING deleted_flag = false;

/* 2. Аксессор получения актуального среза на конкретную версию-дату */
CREATE FUNCTION stg.v_sv_mart_dcm_dict(p_processed_dttm DateTime)
RETURNS TABLE
AS 
    SELECT 
        cd_u,
    ...
    FROM stg.mart_dcm_dict
    WHERE processed_dttm <= p_processed_dttm
    GROUP BY cd_u, stat_year
    HAVING deleted_flag = false;

/* 3. Аксессор получения инкремента для */
CREATE FUNCTION stg.v_iv_mart_dcm_dict(p_from_dttm DateTime, p_to_dttm DateTime)
RETURNS TABLE
AS 
    SELECT 
        cd_u,
...
/* 1. Аксессор получения актуального среза на конкретную дату обработки*/
CREATE VIEW stg.v_sn_mart_dcm_reg
AS 
SELECT 
        cd_reg,
        sales_type_id,
        ...
        argMax(hash_diff, processed_dttm) as hash_diff,
        max(processed_dttm) as processed_dttm
FROM stg.mart_dcm_reg
    GROUP BY cd_reg, sales_type_id
HAVING deleted_flag = false;

/* 2. Аксессор получения актуального среза на конкретную версию-дату */
CREATE FUNCTION stg.v_sv_mart_dcm_reg(p_processed_dttm DateTime)
RETURNS TABLE
AS 
    SELECT 
        cd_reg,
...
/* 3. Аксессор получения инкремента для */
CREATE FUNCTION stg.v_iv_mart_dcm_reg(p_from_dttm DateTime, p_to_dttm DateTime)
RETURNS TABLE
AS 
    SELECT 
        cd_reg,
