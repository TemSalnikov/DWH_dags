...
@dag(
    dag_id='cf_xls_kafka_mart_fpc_vita_tomsk_custom',
    default_args=default_args,
    schedule_interval=None,
    catchup=False,
    params = {'directory': '/opt/airflow/data/reports/Вита Томск/Закуп/',
              'name_report': 'Закупки',
              'name_pharm_chain': 'Вита Томск',
              'prefix_topic': 'fpc_vita_tomsk'
            },
    tags=['advanced', 'vita_tomsk']
)
def cf_xls_kafka_mart_fpc_vita_tomsk_custom():
    @task
    ...
    def trigger_or_skip(parametrs: Optional[Dict], processing_files: Optional[Dict], **context):
        ...
        if processing_files:
            ...
            TriggerDagRunOperator(
                task_id=f"trigger_wf_xls_kafka_mart_fpc_vita_tomsk_custom",
                trigger_dag_id='wf_xls_kafka_mart_fpc_vita_tomsk_custom',
                conf=parametrs
            ).execute(context)
        else:
...
cf_xls_kafka_mart_fpc_vita_tomsk_custom()